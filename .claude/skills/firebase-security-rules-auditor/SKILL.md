...
- Static review only. Any finding must be proven against the Firestore **emulator** with a
  test in `firestore-tests/` before it is called real — never against production (CLAUDE.md #38).
- `firestore.rules` uses a hardcoded admin email for Platform Stats; per this skill's own
  bootstrapping note, check it also requires `email_verified`.
- Report findings to Ani in Cantonese, one line of plain-language consequence per finding.
