...
- Source: `firebase/agent-skills` @ `cad5019`, Apache-2.0 (see `LICENSE`). Installed 2026-09-26.
- **Use for new collections and for rules you are changing — never rewrite the existing
  `firestore.rules` wholesale.** It is live, covered by 69 emulator tests, and encodes
  decisions recorded in CLAUDE.md (immutable `trainerId`/`clientId`/`role`, #22; server-only
  collections such as `subscriptions`, `paymentConnections`, `oauthNonces`).
- Syntax check: do **not** run `firebase deploy … --dry-run` against `elitepro-16718`. Run
  `cd firestore-tests && npm run test:emulator` instead — every rule change needs a test
  there that fails without the change (CLAUDE.md #37 item 6, #38, #40).
- Ignore the `firestore-rules-author` subagent instruction; it is not installed here.
