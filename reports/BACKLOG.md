...
| | |
|---|---|
| **狀態** | 第 1 步（sandbox 帳戶）✅ · Ani 2026-09-23 攞咗一個 **access token** · 第 2 步（partner app）✅ Ani 2026-09-23 建立咗，攞咗 Client ID／Secret · 第 3 步（Secret Manager API）✅ 截圖顯示「API 已啟用」—— 原來之前已經開咗 · 第 4 步（三條 secret）✅ Ani 2026-09-23 話做晒 · 第 5 步（IAM）✅ 同日完成 —— probe 由 `not-configured` 轉做 **`?gc=error`**，即係 function 讀到三條 secret （被拒係因為 probe 用咗假 nonce，屬正常）· 第 6 步：Ani 2026-09-23 11:13 撳咗 Connect —— **成功跳到 `connect-sandbox.gocardless.com`**（即係 gcOAuthStart 讀 secret、砌 URL 全部 work），但 GoCardless 答 **400「An app with the provided client_id does not exist」**。即係 `GC_CLIENT_ID` 個值唔啱 —— ~~最可能係貼咗早前嗰個 access token~~ —— **更正**：Ani 貼咗 authorize URL，解碼後 `client_id='Client ID'`，即係 secret 個值係**欄位名稱本身**（copy 咗標籤，唔係個值）。redirect_uri 解碼後同登記嗰條完全一致 ✅。`GC_CLIENT_SECRET` 好大機會一樣係 `'Client Secret'` 呢幾個字。 → Ani 同日為 `GC_CLIENT_ID` 同 `GC_CLIENT_SECRET` 各加咗新版本（code 讀 `versions/latest`，無 cache，即時生效）；等佢再撳 Connect。修法：Secret Manager → GC_CLIENT_ID → 新增版本，貼返 partner app 頁面嘅 Client ID（code 讀 `versions/latest`，唔使刪舊版本、唔使 redeploy） |
