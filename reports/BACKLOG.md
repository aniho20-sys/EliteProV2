...
> **最後更新**：2026-09-23（✅ A6 GoCardless sandbox 連接成功 · B2 中文 invoice 已修 · B10 等 B11 語言決定 · outreach 無方案）
...
## A6. ✅ GoCardless sandbox 連接 —— Ani 2026-09-23 真機撳 Connect 成功

| | |
|---|---|
| **狀態** | 第 1 步（sandbox 帳戶）✅ · Ani 2026-09-23 攞咗一個 **access token** · 第 2 步（partner app）✅ Ani 2026-09-23 建立咗，攞咗 Client ID／Secret · 第 3 步（Secret Manager API）✅ 截圖顯示「API 已啟用」—— 原來之前已經開咗 · 第 4 步（三條 secret）✅ Ani 2026-09-23 話做晒 · 第 5 步（IAM）✅ 同日完成 —— probe 由 `not-configured` 轉做 **`?gc=error`**，即係 function 讀到三條 secret （被拒係因為 probe 用咗假 nonce，屬正常）· 第 6 步：Ani 2026-09-23 11:13 撳咗 Connect —— **成功跳到 `connect-sandbox.gocardless.com`**（即係 gcOAuthStart 讀 secret、砌 URL 全部 work），但 GoCardless 答 **400「An app with the provided client_id does not exist」**。即係 `GC_CLIENT_ID` 個值唔啱 —— ~~最可能係貼咗早前嗰個 access token~~ —— **更正**：Ani 貼咗 authorize URL，解碼後 `client_id='Client ID'`，即係 secret 個值係**欄位名稱本身**（copy 咗標籤，唔係個值）。redirect_uri 解碼後同登記嗰條完全一致 ✅。`GC_CLIENT_SECRET` 好大機會一樣係 `'Client Secret'` 呢幾個字。 → Ani 同日為 `GC_CLIENT_ID` 同 `GC_CLIENT_SECRET` 各加咗新版本（code 讀 `versions/latest`，無 cache，即時生效）；等佢再撳 Connect。 → 第二次 Connect 彈返「連接失敗」（`?gc=error`，即係去咗 GoCardless 再返嚟之後 server 失敗）。Log 喺手機睇唔到結果（mobile 版面個篩選面板遮住結果清單），所以 2026-09-23 改咗 callback：每個失敗都帶 `reason`（同 GoCardless 嘅 OAuth error code），app 直接顯示原因（commit d128430）。~~下一步：deploy 完 Ani 再撳一次 Connect~~ → 16:57 第三次：新 toast 出咗 `invalid_client`（證明新訊息 work）。今次 GoCardless 授權頁有打開、有批准，即係 **Client ID 已經啱**；`invalid_client` 喺 token exchange 先出，嗰步先用到 Secret，所以係 **`GC_CLIENT_SECRET` 仲唔啱**。最可能：GoCardless 頁面個 Secret 係遮住嘅（•••），copy 咗啲圓點；或者又 copy 咗標籤。~~下一步：喺 GoCardless 重新顯示 Secret~~ → 17:02 截圖證實：GoCardless 個 Client secret 欄**只顯示「Reveal」連結**，唔撳就冇值可以 copy —— 之前兩次 copy 到嘅都唔係 Secret。Ani 撳 Reveal 後為 `GC_CLIENT_SECRET` 加咗新版本。~~等第四次 Connect~~ → **第四次成功（Ani 2026-09-23）**。由 8 月 deploy 到今日，第一次真係連得通。可選清理（唔急、冇風險）：GoCardless 較早開嗰個 access token 可以 revoke；Secret Manager 入面寫錯嗰幾個舊版本可以停用。修法：Secret Manager → GC_CLIENT_ID → 新增版本，貼返 partner app 頁面嘅 Client ID（code 讀 `versions/latest`，唔使刪舊版本、唔使 redeploy） |
...
| ✅ **B2** | **invoice PDF 中文爆** | 2026-09-14 | `SA-report-2026-09-14` | **已完成 2026-09-22** —— 中文名嘅 invoice 出到 PDF，成個檔案約 25 KB。順手揪到第二個 bug：中文冇空格，舊嘅換行邏輯當成一個字，長描述會衝出版面 —— 一併修咗。⚠️ **等你驗收**：開一張 invoice，客戶名／描述打中文，撳出 PDF 睇下出唔出到、有冇衝出版面 |
...
| ✅ **B12** | **Phase 3 值唔值得起落去 —— 定係直接用 GoCardless 收錢** | 2026-09-23 | Ani 問「學生都係去 GoCardless 俾錢，做咩要連 ElitePro」 | 收錢本身唔需要 ElitePro，GoCardless 自己個 dashboard 做得到訂閱。ElitePro 連接加嘅係：**扣款成功自動加堂**、**扣款失敗自動預警**、roll-over／暫停規則、學生喺同一個 app 睇晒。9 個學生手動對數做得到；對其他教練，呢個係賣點。⚠️ 真錢要 live partner 審批（GoCardless 冇公佈時間），直接開 live 商戶帳戶可能更快收到錢。~~等 Ani 決定~~ → **Ani 2026-09-23 決定：繼續起 Phase 3。** 原話：「我目的係想做一個安全高嘅可以經 app 俾埋錢嘅教練管理平台」—— app 內付款係產品核心，唔係可有可無。下一步 Phase 3 Step 3（學生訂閱畫面 + GoCardless 授權）。 |
| **B13** | 🔴 **Phase 3 Step 3 真機驗收（學生訂閱）** | 2026-09-23 | commit 00185b3 | 要一個連住 Ani 嘅測試學生帳戶（唔好用真學生）。Ani：個人檔案設每堂收費 → 學生頁撳「標記為測試者」→ 用測試學生登入撳訂閱 → GoCardless sandbox 填測試銀行（20-00-00 / 55779911）→ 返嚟見「已啟用」。**完成標準：測試學生張卡顯示已啟用 + 月費金額。** 注意：堂數要 Step 4 先會自動加。<br>**進度：Ani 2026-09-23 話「成功咗」—— 教練端兩步做完,學生 Profile 出咗「每月訓練計劃」卡。** 剩低：學生撳訂閱 → sandbox 銀行 → 返嚟見「已啟用」。<br>教訓：第一次交接只寫咗「設每堂收費 → 標記測試者」,冇講喺邊度撳(收費喺 Profile →「GoCardless 連接」格入面;測試者掣喺學生「概覽」→「標籤」下面,而且**要先儲存收費先會出現**),Ani 淨係睇學生頁,見唔到卡來回兩次。違反常規 #43 第 3 條。 |
...
| **B16** | ❓ **owner-only 功能要唔要加 `email_verified` 檢查** | 2026-09-26 | rules auditor | Platform Stats、清假帳號只對 email。要先知 Ani 用 Google 定 email+密碼登入 —— 加錯會鎖死自己 |
