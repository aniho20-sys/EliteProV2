...
**狀態**：✅ **Ani 已拍板 2026-09-11，可以動工**
**日期**：2026-09-09 出草案 · 2026-09-11 拍板定稿
**作者**：員工A（SA）
**相關**：`ROADMAP.md` Phase 1、`reports/phase3-subscription-design.md`、常規 #26 / #27 / #33 / #35 / #36 / #37 / #38 / #40

> 八條決定喺第 6 節。第 3 節係落實規格，直接照住做。
...
2. Top-up 對話框加「有效期」下拉，四個選項：**3 / 6 / 12 個月 / 無期限**，
   **預設揀「無期限」**（Ani 2026-09-11 指定）。
   **⚠️ 呢個預設令功能變成「預設關閉」** —— 唔主動揀就冇期限。
   係刻意嘅 fail-safe：跳過個下拉唔會意外幫學生加咗期限，
   同第 5 節「空白 = 永不過期」同一個方向。
   **補償措施**：揀咗非「無期限」時，UI 要顯示一行確認摘要
   （例：「呢批 10 堂會喺 2026-12-11 到期」），
   令「揀咗」同「冇揀」兩種情況睇得出分別。
...
| # | 問題 | **決定** | 員工A 原建議 |
|---|---|---|---|
| 1 | 開始日 | **1a — 第一次 book 起計** | 1a ✅ 一致 |
| 2 | 架構 | **B — 逐次填期限** | B ✅ 一致 |
| 3 | 過期處理 | **3b — 凍結** | 3b ✅ 一致 |
| 4 | 訂閱制 | **豁免，唔加到期日** | 豁免 ✅ 一致 |
| 5 | 現有客 | **空白 = 永不過期** | 同意 ✅ 一致 |
| 6 | Needs Attention | **加，排序交員工D 決定**；push notification 30 日 / 7 日兩次都要 | 加 ✅ 一致 |
| 附1 | 早取消唔回退開始日 | **接受** | — |
| 附2 | 寬限期 | **30 日** | 30 日 ✅ 一致 |
| 新增 | 期限選項 | **3 / 6 / 12 個月 / 無期限，預設「無期限」** | Ani 自己加 |

**Ani 對問題 2 嘅理由（原話記低）**：
> 9 個客得 1 個有兩批，唔值得為咗佢重寫已上線嘅收費核心。

**方案 C 唔係否決咗，係押後。** 觸發條件：當「同一個客持有多批唔同期限堂數」
由 1 個客變成常態（例如外部教練上場之後），或者第 5 節嗰個
「現有客加堂之後表達唔到」嘅限制開始有實際爭議，就要重開 C。

---

_內部設計文件（已批准），Cantonese working doc — 見 CLAUDE.md「Working Rules」_
